...
'''
    ...
    fname = "%s_%d.py" % (model, sample_domain)
    
    fptr = open(fname, 'w')
    fptr.write(local)
    fptr.close()
    
    pysub(fname, queue=QUEUE, nprocs=NPROCS, use_mpi=False, walltime=WALLTIME, priority=20, modules=["pal"], jobarray=RANGE)
    print("Submitting %s" % fname)
    time.sleep(0.1)
...
if __name__ == "__main__":
    ...
    for sample_domain in [1000]:
    #for sample_domain in [250, 500, 1000]:
        if sample_domain == 1000:
            WALLTIME = "3-0:0:0"
        if not os.path.exists(folder + "_" + str(sample_domain)):
            os.mkdir("%s_%d" % (folder, sample_domain))
        run_pricm("%s_%d" % (folder, sample_domain), RANGE=RANGE, QUEUE=QUEUE, WALLTIME=WALLTIME, sample_domain=sample_domain)
#        run_ei("%s_%d" % (folder, sample_domain), RANGE=RANGE, QUEUE=QUEUE, WALLTIME=WALLTIME, sample_domain=sample_domain)
